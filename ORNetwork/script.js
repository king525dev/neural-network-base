...
function trainLoop() {
    ...
    if (epochs < MAX_EPOCHS) {
        ...
        logMessage(`<span class="done">DONE!</span> Testing OR gate:`);
        ...
        data.forEach(d => {
            ...
            logMessage(
                `  ${d.x1} OR ${d.x2} = ${prediction} (raw: ${p.toFixed(4)})`
                );
        ...
        if (passed) {
            logMessage(`<span style="color: #3fb950;"> OR Perceptron PASSED</span>`)
        } else {
            logMessage(`<span style="color: #b93f3f;"> OR Perceptron FAILED</span>`)
        }
